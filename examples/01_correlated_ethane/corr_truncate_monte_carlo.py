...
#
# decomposed space truncate that DOES respect correlations
...
# +
# compare truncation pdfs

colors = plt.rcParams['axes.prop_cycle'].by_key()['color']

name = condition['name']

uncorr_truncate = np.load('monte_carlo/uncorr_truncate_900K/monte_carlo_results.npy')
corr_truncate = np.load('monte_carlo/corr_truncate_900K/monte_carlo_results.npy')

uncorr_truncate = uncorr_truncate[~np.all(uncorr_truncate == 0, axis=1)]
uncorr_truncate = uncorr_truncate[~np.isnan(uncorr_truncate[:, i_sp])]
corr_truncate = corr_truncate[~np.all(corr_truncate == 0, axis=1)]
corr_truncate = corr_truncate[~np.isnan(corr_truncate[:, i_sp])]

result = plt.hist(uncorr_truncate[:, i_sp], 48, density=True, alpha=0.6, histtype='step', color=colors[0], label='Parameter Truncate')
result = plt.hist(corr_truncate[:, i_sp], 48, density=True, alpha=0.6, histtype='step', color=colors[1], label='Decomposed Truncate')
plt.axvline(x=np.mean(nominal_results[1, i_sp]), label='Nominal Value', color='k', linewidth=0.4)
